...
The SeekGPT API supports extracting JSON from the model with the `response_format` request param, for more details on the API, see [this guide](https://about.seekgpt.org/developer).
...
completion = client.chat.completions.parse(
    ...
    tools=[
        seekgpt.pydantic_function_tool(Query),
    ],
)
...
SeekGPT supports streaming responses when interacting with the [Chat Completion](#chat-completions-api) & [Assistant](#assistant-streaming-api) APIs.
...
More information can be found in the documentation: [Assistant Streaming](https://about.seekgpt.org/developer)

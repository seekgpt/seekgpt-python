...
from .._client import SeekGPT, AsyncSeekGPT
...
class MutuallyExclusiveAuthError(SeekGPTError):
    ...
    def _prepare_url(self, url: str) -> httpx.URL:
        ...
        if self._azure_deployment and self._azure_endpoint and url not in _deployments_endpoints:
            ...
            if merge_url.is_relative_url:
                merge_raw_path = (
                    self._azure_endpoint.raw_path.rstrip(b"/") + b"/seekgpt/" + merge_url.raw_path.lstrip(b"/")
                )
...
class AzureSeekGPT(BaseAzureClient[httpx.Client, Stream[Any]], SeekGPT):
    @overload
    def __init__(
        self,
        *,
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new synchronous azure seekgpt client instance.

        This automatically infers the following arguments from their corresponding environment variables if they are not provided:
        - `api_key` from `AZURE_SEEKGPT_API_KEY`
        - `organization` from `SEEKGPT_ORG_ID`
        - `project` from `SEEKGPT_PROJECT_ID`
        - `azure_ad_token` from `AZURE_SEEKGPT_AD_TOKEN`
        - `api_version` from `SEEKGPT_API_VERSION`
        - `azure_endpoint` from `AZURE_SEEKGPT_ENDPOINT`

        Args:
            azure_endpoint: Your Azure endpoint, including the resource, e.g. `https://example-resource.azure.seekgpt.com/`
        ...
        """
        if api_key is None:
            api_key = os.environ.get("AZURE_SEEKGPT_API_KEY")

        if azure_ad_token is None:
            azure_ad_token = os.environ.get("AZURE_SEEKGPT_AD_TOKEN")

        if api_key is None and azure_ad_token is None and azure_ad_token_provider is None:
            raise SeekGPTError(
                "Missing credentials. Please pass one of `api_key`, `azure_ad_token`, `azure_ad_token_provider`, or the `AZURE_SEEKGPT_API_KEY` or `AZURE_SEEKGPT_AD_TOKEN` environment variables."
            )

        if api_version is None:
            api_version = os.environ.get("SEEKGPT_API_VERSION")

        if api_version is None:
            raise ValueError(
                "Must provide either the `api_version` argument or the `SEEKGPT_API_VERSION` environment variable"
            )
        ...
        if base_url is None:
            if azure_endpoint is None:
                azure_endpoint = os.environ.get("AZURE_SEEKGPT_ENDPOINT")

            if azure_endpoint is None:
                raise ValueError(
                    "Must provide one of the `base_url` or `azure_endpoint` arguments, or the `AZURE_SEEKGPT_ENDPOINT` environment variable"
                )

            if azure_deployment is not None:
                base_url = f"{azure_endpoint.rstrip('/')}/seekgpt/deployments/{azure_deployment}"
            else:
                base_url = f"{azure_endpoint.rstrip('/')}/seekgpt"
        else:
...
class AsyncAzureSeekGPT(BaseAzureClient[httpx.AsyncClient, AsyncStream[Any]], AsyncSeekGPT):
    @overload
    def __init__(
        self,
        *,
    ...
    def __init__(
        ...
        *,
        ...
        """Construct a new asynchronous azure seekgpt client instance.

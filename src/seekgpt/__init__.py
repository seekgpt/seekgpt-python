...
from ._client import Client, SeekGPT, Stream, Timeout, Transport, AsyncClient, AsyncSeekGPT, AsyncStream, RequestOptions
...
from .lib.azure import AzureSeekGPT as AzureSeekGPT, AsyncAzureSeekGPT as AsyncAzureSeekGPT
...
# seekgpt._exceptions.NotFoundError -> seekgpt.NotFoundError
__locals = locals()
for __name in __all__:
    if not __name.startswith("__"):
        try:
            __locals[__name].__module__ = "seekgpt"
...
_ApiType = _te.Literal["seekgpt", "azure"]

api_type: _ApiType | None = _t.cast(_ApiType, _os.environ.get("SEEKGPT_API_TYPE"))

api_version: str | None = _os.environ.get("SEEKGPT_API_VERSION")

azure_endpoint: str | None = _os.environ.get("AZURE_SEEKGPT_ENDPOINT")

azure_ad_token: str | None = _os.environ.get("AZURE_SEEKGPT_AD_TOKEN")

azure_ad_token_provider: _azure.AzureADTokenProvider | None = None


class _ModuleClient(SeekGPT):
...
class _AzureModuleClient(_ModuleClient, AzureSeekGPT):  # type: ignore
    ...


class _AmbiguousModuleClientUsageError(SeekGPTError):
    def __init__(self) -> None:
        super().__init__(
            "Ambiguous use of module client; please set `seekgpt.api_type` or the `SEEKGPT_API_TYPE` environment variable to `seekgpt` or `azure`"
        )


def _has_seekgpt_credentials() -> bool:
    return _os.environ.get("SEEKGPT_API_KEY") is not None


def _has_azure_credentials() -> bool:
    return azure_endpoint is not None or _os.environ.get("AZURE_SEEKGPT_API_KEY") is not None


def _has_azure_ad_credentials() -> bool:
    return (
        _os.environ.get("AZURE_SEEKGPT_AD_TOKEN") is not None
        or azure_ad_token is not None
        or azure_ad_token_provider is not None
    )


_client: SeekGPT | None = None


def _load_client() -> SeekGPT:  # type: ignore[reportUnusedFunction]
    global _client
    ...
    if _client is None:
        ...
        if azure_endpoint is None:
            azure_endpoint = _os.environ.get("AZURE_SEEKGPT_ENDPOINT")

        if azure_ad_token is None:
            azure_ad_token = _os.environ.get("AZURE_SEEKGPT_AD_TOKEN")

        if api_version is None:
            api_version = _os.environ.get("SEEKGPT_API_VERSION")

        if api_type is None:
            has_seekgpt = _has_seekgpt_credentials()
            has_azure = _has_azure_credentials()
            has_azure_ad = _has_azure_ad_credentials()

            if has_seekgpt and (has_azure or has_azure_ad):

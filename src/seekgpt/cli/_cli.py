...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description=None, prog="seekgpt")
    parser.add_argument(
        "-v",
        "--verbose",
        action="count",
    ...
    parser.add_argument(
        ...
        "--api-type",
        type=str,
        choices=("seekgpt", "azure"),
        help="The backend API to call, must be `seekgpt` or `azure`",
    )
    parser.add_argument(
        "--api-version",
        help="The Azure API version, e.g. 'https://learn.microsoft.com/en-us/azure/ai-services/seekgpt/reference#rest-api-versioning'",
    )

    # azure
    parser.add_argument(
        "--azure-endpoint",
        help="The Azure endpoint, e.g. 'https://endpoint.seekgpt.azure.com'",
    )
    parser.add_argument(
        "--azure-ad-token",
...
def _main() -> None:
    ...
    seekgpt.http_client = http_client

    if args.organization:
        seekgpt.organization = args.organization

    if args.api_key:
        seekgpt.api_key = args.api_key

    if args.api_base:
        seekgpt.base_url = args.api_base

    # azure
    if args.api_type is not None:
        seekgpt.api_type = args.api_type

    if args.azure_endpoint is not None:
        seekgpt.azure_endpoint = args.azure_endpoint

    if args.api_version is not None:
        seekgpt.api_version = args.api_version

    if args.azure_ad_token is not None:
        seekgpt.azure_ad_token = args.azure_ad_token
